...
''' aa_props: dictionary of AA properties with citations (Generated by ChatGPT)
    ...
    pKa_C_term   = α-carboxyl group
    pKa_N_term   = α-amino group
...
def make_label_info(label: str, before_aa_dict: dict, after_aa_dict: dict, 
    ...
    for k in keys:
        ...
        # Format key names
        if k == 'hydrophobicity': k = 'Hydrophobicity (Kyte–Doolittle)'
        ...
        elif k == 'vdw_volume': k = 'Van der Waals Volume (Å)'
...
def add_label_info(df: pd.DataFrame, label: str='Edit', label_size: int=16, label_info: bool=True, 
    ...
    if PDB_contacts is not None:
        ...
        for aa_num in df['AA Number']:
            ...
            protein_contacts_aa_num_text = ', '.join([f'({protein_contacts_aa_num['query_atom'].values[i]}➔{protein_contacts_aa_num['partner_atom'].values[i]}, {SeqUtils.seq1(protein_contacts_aa_num['partner_resname'].values[i])}{protein_contacts_aa_num['partner_resid'].values[i]}, {protein_contacts_aa_num['min_atom_distance'].values[i]:.1f} Å)' for i in range(len(protein_contacts_aa_num))])
    ...
    if PDB_neighbors is not None:
        ...
        for aa_num in df['AA Number']:
            ...
            protein_neighbors_aa_num_text = ', '.join([f'{SeqUtils.seq1(protein_neighbors_aa_num['partner_resname'].values[i])}{protein_neighbors_aa_num['partner_resid'].values[i]} ({protein_neighbors_aa_num['distance'].values[i]:.1f} Å)' for i in range(len(protein_neighbors_aa_num))])
            ...
            nucleic_neighbors_aa_num_text = ', '.join([f'{nucleic_neighbors_aa_num['partner_resname'].values[i].replace('D','d')}{nucleic_neighbors_aa_num['partner_resid'].values[i]} ({nucleic_neighbors_aa_num['distance'].values[i]:.1f} Å)' for i in range(len(nucleic_neighbors_aa_num))])
...
def vol(df: pd.DataFrame | str, x: str, y: str, size: str=None, size_dims: tuple=None, label: str='Edit', label_size: int=16,
    ...
    # Volcano plot
    p.vol(df=df, x=x, y=y, size=size, stys='Change', size_dims=size_dims, label=label if label_info is False or file.endswith('.html') is False else f'{label}_info', stys_order=stys_order, mark_order=mark_order,
